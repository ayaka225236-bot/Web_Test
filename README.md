...
| 区域 | 组件 | 说明 |
| --- | --- | --- |
| 左上角导航 | `NavDrawer.tsx` | 图标固定在左上角，**鼠标悬停即滑出**；内含**站点切换**与当前站点的条目 |
...
站点登记在 `data/sites.ts`：一个站点 = 一个条目，下面挂着它自己的页面列表。
...
```ts
{
  ...
  href: "/",                    // 切换时点击的目标
  pages: [                      // 该站点自己的条目
    { href: "/", label: "首页", group: "概览", order: 1 },
  ],
}
```

抽屉顶部显示当前站点名 + 一个"切换站点（N）"按钮，**点开**才列出其它站点，
这样抽屉默认保持紧凑。`defaultSiteId` 指定当前站点，`otherSites` 是切换菜单的内容。

现在 `data/sites.ts` 里的第二、三个站点是**占位条目**（指向本站已有页面），
等你真的做出第二个站点，把它的 `pages` 挂上、`href` 改成真实地址即可，结构不用动。

### 导航抽屉的交互

- 鼠标**悬停到图标上**就展开侧边栏；
- 鼠标从图标移到侧边栏上**保持展开**，可以点击里面的条目与"切换站点"；
- 鼠标离开"图标 + 面板"这整块区域才收起；
